...
const ensureBacksideSchema = async (queryFn, companyId) => {
  ...
  if (ensuredBacksideSchemas.has(cacheKey)) return;

  try {
    const columns = [
      { name: 'annexure_id', type: 'UUID' },
      { name: 'c_no', type: 'TEXT' },
      ...
      { name: 'branch_code_no', type: 'TEXT' },
      { name: 'bin_no', type: 'TEXT' },
      { name: 'weighbridge_name', type: 'TEXT' },
      { name: 'max_permissible_weight', type: 'TEXT' },
      { name: 'cargo_type', type: 'TEXT' },
      { name: 'country_of_origin', type: 'TEXT' },
      { name: 'is_description_match', type: 'TEXT' },
      ...
      { name: 'voyage_no', type: 'VARCHAR(100)' },
      { name: 'backside_no', type: 'VARCHAR(100)' },
      { name: 'pi_no', type: 'VARCHAR(255)' },
      { name: 'pl_no', type: 'VARCHAR(255)' },
      { name: 'annexure_invoice_no', type: 'VARCHAR(100)' },
      { name: 'exporter_name', type: 'VARCHAR(255)' },
      { name: 'exporter_address', type: 'TEXT' },
      { name: 'iec_code', type: 'VARCHAR(50)' },
      { name: 'survey_no', type: 'VARCHAR(100)' },
      { name: 'consignee_name', type: 'VARCHAR(255)' },
      { name: 'consignee_address', type: 'TEXT' },
      { name: 'goods_value_match', type: 'VARCHAR(50)' },
      { name: 'permission_year', type: 'VARCHAR(20)' },
      { name: 'contact_details', type: 'VARCHAR(255)' },
      { name: 'consignee_contact', type: 'VARCHAR(255)' },
      { name: 'notify_party', type: 'VARCHAR(255)' },
      { name: 'notify_party_address', type: 'TEXT' },
      { name: 'notify_party_contact', type: 'VARCHAR(255)' },
      { name: 'etd', type: 'DATE' },
      { name: 'eta', type: 'DATE' },
      { name: 'pod', type: 'DATE' },
      { name: 'booking_no', type: 'VARCHAR(100)' },
      { name: 'hbl_no', type: 'VARCHAR(100)' },
      { name: 'freight_terms', type: 'VARCHAR(255)' },
      { name: 'place_of_delivery', type: 'VARCHAR(255)' },
      { name: 'total_packages', type: 'INTEGER DEFAULT 0' },
      { name: 'is_locked', type: 'BOOLEAN DEFAULT false' }
    ];

    for (const col of columns) {
      await queryFn(
        `ALTER TABLE invoice_backside ADD COLUMN IF NOT EXISTS ${col.name} ${col.type}`
      );
    }
    ensuredBacksideSchemas.add(cacheKey);
  } catch (err) {
    debugLogger.error('[Backside Schema Self-Healing] Error:', err.message);
  }
};
