...
import { Card, Row, Col, Table, Button, Form, Modal, Badge, Spinner, InputGroup, Tabs, Tab } from 'react-bootstrap';
...
import GRNList from './GRNList.jsx';
import StockLedger from './StockLedger.jsx';
...
const InventoryDashboard = ({ onNavigate, showSuccess, showError }) => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [summaryRes, stockRes, movRes, resvRes, prodRes, sanProdRes, whRes] = await Promise.all([
      ...
      if (whList.length > 0) {
        setForm(prev => ({ ...prev, warehouse_location: prev.warehouse_location === 'Main Warehouse' ? whList[0].name : prev.warehouse_location }));
      }
    } catch (err) {
    ...
    <div className="p-3">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">Track stock, GRNs, ledgers, and warehouses in one place</p>
        </div>
        <div className="d-flex gap-2">
          ...
          <Button variant="primary" onClick={() => setShowModal(true)}>Manual Movement</Button>
        </div>
      </div>
      ...
      <Tabs
        ...
        className="mb-4"
      >
        <Tab eventKey="overview" title="Stock Overview">
        ...
        <Tab eventKey="grn" title="Goods Receipt (GRN)">
          <GRNList showSuccess={showSuccess} showError={showError} warehouses={warehouses} products={products} />
        </Tab>

        <Tab eventKey="ledger" title="Stock Ledger">
          <StockLedger showError={showError} products={products} warehouses={warehouses} />
        </Tab>

        <Tab eventKey="warehouses" title="Warehouses">
          <WarehouseManagement showSuccess={showSuccess} showError={showError} />
        </Tab>

      </Tabs>

      <Modal show={showModal} onHide={() => setShowModal(false)}>
        <Modal.Header closeButton><Modal.Title>Record Manual Movement</Modal.Title></Modal.Header>
        <Form onSubmit={handleMovement}>
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Select required value={form.warehouse_location}
                onChange={(e) => setForm({ ...form, warehouse_location: e.target.value })}>
                {warehouses.map(w => (
                  <option key={w.id} value={w.name}>{w.name}</option>
                ))}
                {warehouses.length === 0 && <option value="Main Warehouse">Main Warehouse</option>}
              </Form.Select>
            </Form.Group>
            <Form.Group>
