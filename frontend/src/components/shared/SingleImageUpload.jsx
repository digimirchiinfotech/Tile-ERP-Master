...
function SingleImageUpload({
  ...
  const imageUrl = image ? resolveImageUrl(image.url || image) : '';
  const imageName = image ? (image.name || (typeof image === 'string' ? image.split('/').pop() : 'Image')) : '';
  const imageSize = image && image.size ? image.size : null;
  const imageType = image && image.type ? image.type : 'Image';
        ...
        {!image ? (
          ...
          <Card className="image-preview-card">
            ...
            <Card.Body className="p-3">
              <h6 className="mb-1 text-truncate" title={imageName}>
                {imageName}
              </h6>
              <small className="text-muted">{formatFileSize(imageSize)}</small>
              <div className="mt-2">
                <Button
                  variant="outline"
                  size="sm"
            ...
            <div>
              ...
              <div className="mt-3">
                ...
                <p className="text-muted">
                  Size: {formatFileSize(imageSize)} | Type: {imageType}
                </p>
              </div>
            </div>
          )}
