  ...
  try {
    const checkQuery = `
      SELECT EXISTS (
        ...
        AND table_name = 'sanitaryware_products'
      );
    `;
    const { rows } = await queryFn(checkQuery);
    if (!rows[0].exists) {
      debugLogger.info('[Sanitaryware Schema] Table sanitaryware_products missing. Creating dynamically...');
      await queryFn(`
        CREATE TABLE IF NOT EXISTS public.sanitaryware_products (
          ...
          description TEXT,
          category VARCHAR(100), -- Pedestal Basin, One Piece WC, Two Piece WC, etc.
          brand VARCHAR(100),
      ...
      `);
      debugLogger.info('[Sanitaryware Schema] Table sanitaryware_products created successfully!');
    } else {
      // Dynamic auto-migration: drop the removed fields
      await queryFn(`
        ALTER TABLE public.sanitaryware_products DROP COLUMN IF EXISTS cbm_per_piece;
        ALTER TABLE public.sanitaryware_products DROP COLUMN IF EXISTS master_carton_qty;
      `);
      
      // Auto-migrate hs_code to hsn_code
      const hsnCheckQuery = `
        SELECT column_name 
        FROM information_schema.columns 
        WHERE table_name='sanitaryware_products' AND column_name='hsn_code';
      `;
      const hsnCheck = await queryFn(hsnCheckQuery);
      if (hsnCheck.rows.length === 0) {
        debugLogger.info('[Sanitaryware Schema] Migrating hs_code to hsn_code...');
        await queryFn(`
          ALTER TABLE public.sanitaryware_products RENAME COLUMN hs_code TO hsn_code;
        `);
      }
    }
  } catch (err) {
    debugLogger.error('[Sanitaryware Schema] Error ensuring sanitaryware_products exists:', err.message);
  }
};
