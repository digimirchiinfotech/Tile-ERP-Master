...
function ProductDashboard({ currentUser, productsData, navigationData }) {
  ...
  const exportProducts = () => {
    const columns = [
      ...
      createColumnDef('Factory Product Name', 'factoryProductName'),
      createColumnDef('Product Name', (p) => p.companyProductName || p.name || ''),
      createColumnDef('Description', 'description'),
      createColumnDef('Product Code', 'productCode'),
      createColumnDef('Catalogue Name', 'catalogueName'),
      ...
      createColumnDef('Thickness', (p) => Array.isArray(p.thickness) ? p.thickness.join(', ') : (p.thickness || '')),
      createColumnDef('Application', (p) => Array.isArray(p.application) ? p.application.join(', ') : (p.application || '')),
      createColumnDef('HSN Code', 'hsCode'),
      createColumnDef('Box PCS', 'boxPcs'),
      createColumnDef('SQM Per Box', 'sqmPerBox'),
      createColumnDef('Boxes Per Big Pallet', 'defaultBoxesPerPallet'),
      createColumnDef('Boxes Per Kathali', 'defaultBoxesPerKathali'),
      createColumnDef('Per Box Weight (KG)', 'boxWeight'),
      createColumnDef('Per Pallet Weight (KG)', 'defaultPerPalletWeight'),
      createColumnDef('Image URL', (p) => p.images?.[0]?.url || p.images?.[0]?.path || ''),
    ];
  ...
  const handleImportData = async (importData) => {
    try {
      ...
      const mappedProducts = importData.map(prod => ({
        ...
        company_product_name: prod.companyProductName || prod['Product Name'] || prod['Company Product Name'] || prod.name || null,
        ...
        item_ref: prod.itemRef || prod['Product Code'] || null,
        catalogue_name: prod.catalogueName || prod['Catalogue Name'] || prod['Catalogue'] || 'Tiles',
        category: prod.category || prod['Category'] || 'Tiles',
        size: prod.size || prod['Size'] || null,
        surface: prod.surface || prod['Surface'] || null,
        thickness: prod.thickness || prod['Thickness'] || null,
        application: prod.application || prod['Application'] || null,
        hs_code: prod.hsCode || prod['HSN Code'] || prod['HS Code'] || null,
        box_pcs: prod.boxPcs || prod['Box PCS'] || 0,
        sqm_per_box: prod.sqmPerBox || prod['SQM Per Box'] || 0,
        box_weight: prod.boxWeight || prod['Per Box Weight (KG)'] || 0,
        default_per_box_weight: prod.defaultPerBoxWeight || prod['Per Box Weight (KG)'] || 0,
        boxes_per_pallet: prod.boxesPerPallet || prod['Boxes Per Big Pallet'] || 0,
        default_boxes_per_kathali: prod.defaultBoxesPerKathali || prod['Boxes Per Kathali'] || 0,
        default_per_pallet_weight: prod.defaultPerPalletWeight || prod['Per Pallet Weight (KG)'] || 0,
        status: 'Active',
